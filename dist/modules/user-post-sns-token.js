...
function default_1(req, res) {
    schemas_1.User.findById(req.user._id, (err, user) => {
        try {
            ...
            if (!device)
                throw 'device not found, something went wrong';
            user.devices.set(req.headers.uuid, Object.assign({}, device, { token,
                os }));
            schemas_1.User.updateOne(user, (err, savedUserData) => {
                if (err)
                    throw 'couldn\'t save sns token';
...
//# sourceMappingURL=user-post-sns-token.js.map