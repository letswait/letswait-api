  ...
  if(users && users.length) {
    ...
    if(users && users.length) {
      query.location = { $near: {
        $geometry: {
          type: 'Point',
          coordinates: [users[0].lastLocation[0],users[0].lastLocation[1]],
        },
        $maxDistance: 500000,
      }}
      ...
      const count = venues.length
        ...
        let foundSegments = 0
        const chosenSegment = Math.min(Math.floor(Math.random() * 8), 7)
        let segments = new Array(8).fill(undefined)
            ...
            segments[i] = {
              ...
              ...(i === chosenSegment ? {
                code: (() => {
                  ...
                  const firstIndexPairValue = Math.min(Math.random() * Math.min(10, venue.codeValidationSecret.sum), 9)
                  return new Array(6).fill(0).map((num, i, arr) => {
                    ...
                    return Math.min(Math.random() * 10, 9)
                  }).join('')
                })(),
              } : null),
            }
