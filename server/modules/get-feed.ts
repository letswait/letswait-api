...
export default async function (req, res) {
  const feed = await getFeed(req.user._id)
  if(feed) {
    res.status(200).send({
      feed,
    })
...
export async function getFeed(requestUserId: mongoose.Types.ObjectId) {
  try {
    // Get Queued Matches
    const user = await User.findOne({ _id: requestUserId }).populate({
      path: 'matches',
      match: { state: 'queued' },
      options: { lean: true },
    })
    if (!user || !user.lastLocation) {
      throw new Error('couldnt get feed')
    } else {
      let feed: any = []
      ...
      if (feed.length <= 10) {
        ...
        const userWithMatches = await User.findOne({ '_id': user._id }, 'matches').populate({
          path: 'matches',
          options: { lean: true },
        }).lean()
        const matchRestrictor = [user._id, ...userWithMatches.matches.map((match, i, arr) => {
        ...
        if (newMatches && newMatches.length) {
          ...
          const userMatches: string[] = newMatches.map((match, i, arr) => {
            if (newBotAccounts.length) {
              const candidateId = match.userProfiles[0] === user._id ?
          ...
          await User.updateOne({ _id: user._id }, { $push: { matches: userMatches } })
        }
      }
      ...
      const userIds = feed.map((match, i, arr) => {
        ...
        const userId = idKeys[0] === user._id ? idKeys[0] : idKeys[1]
      ...
      //    Aggregate Scores
      //    Sort List By Raw Fitness Value 0-1
